...
- **Completion protocol (one-line reference; normative rule in manifest §1.8).** No worker self-completes work whose review class requires independent review: post the handoff evidence as a comment and block with `review-required:`; terminal completion is applied by the authorized owner only after the review gate passes (OM §6.1, org ADR-004 §54-8). Existing M0 cards are covered operationally by the OM §6.1 watchdog audit; no board edits are required.
- **Task-sizing discipline (one-line reference; normative rule in manifest §1b/§1.9).** Remaining M0 packages carry agent-turn sizing estimates tied to lane budgets (senior ≤ ~63 turns, junior ≤ ~42 turns); oversized packages are split into multiple cards in the manifest, each with its own owned area, acceptance criteria, review class, and serial edges; actuals are recorded per package after completion. The Coordinator supersedes original oversized cards for unrun packages per manifest §5.
- **Sizing recalibration (one-line reference; normative rule in manifest §1b/§5, amendment 2026-08-11).** Estimates are recalibrated against the WP-M0-11a..12a actuals: delivered LOC ≈ 3–5× naive estimate and ≈13–17 LOC/turn for compiler-core packages. The senior lane budget is recommended to rise from 90 to 150 turns (threshold ≈105); remaining over-threshold packages (WP-M0-12b..19c) are pre-split into second-level cards (12b1/12b2 … 19c1/19c2) in the manifest, and the Coordinator applies/supersedes per manifest §5 before dispatch.
