# AI-Co Language Specification v0.1.6 (Accepted)
...
- `default` is optional; if present it may appear once, in any position among cases (position is semantically irrelevant; cases are matched in source order, then default). A duplicate `default` clause is **rejected** with `AIC-E0420`.
...
- [x] Requirements traceability to the accepted ADRs (ADR-001/002/004/005) and the charter — §19.
- [x] Draft does not make implementation decisions owned by Specialists or architecture decisions owned by the Main Designer — decisions are constrained to elaboration within ADR-001/002/004/005; no false decision attribution and no nonexistent decision records are referenced.
...
- **v0.1.6 (2026-08-12):** MIN-1 closure from independent review t_ac7567f7 / gate t_61681c3a (follow-up to the v0.1.5 §15.2 alignment, commit bb71338): §20 self-checklist now enumerates ADR-005 in both the traceability line and the decision-constraint line (ADR-001/002/004/005). No other accepted-ADR enumeration was left stale: header authority item 6, §19 traceability row, and §20 line 1223 already carried ADR-005. No production-source change, no corpus, no DIAGNOSTIC-CONTRACT, no ADR change. Authority: Planner per OM §6.1 review-finding remediation (reviewer-specified resolution, owner Planner); no new architecture decision, no Human Sponsor gate. Reviewer conformance re-review pending per OM §6.1.
